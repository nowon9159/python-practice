...
과제처럼 제출해서 AI 검토를 받으려면:

1. 이 폴더의 **`SUBMIT.md`** 를 연다.
2. 체크리스트에 완료한 항목은 `[x]`로 표시한다.
3. **검토용 정보** 칸에 터미널 실행 결과를 붙여넣는다(권장).
4. AI에게 **"Day 1 제출 검토해줘"** 라고 요청한다.

다른 Day는 `templates/SUBMIT_TEMPLATE.md`를 복사해 해당 Day 체크리스트에 맞게 수정해 쓰면 된다.
