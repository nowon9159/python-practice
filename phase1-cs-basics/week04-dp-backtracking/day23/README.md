...
- **2D DP 흐름**: (1) **정의** — dp[i][j]가 "무엇의 값인지"를 명확히 둔다(예: text1[0:i], text2[0:j]까지의 LCS 길이). (2) **초기값** — 한쪽 길이가 0일 때(보통 0). (3) **전이** — 현재 칸이 이전 칸들(위, 왼쪽, 대각선)에서 어떻게 유도되는지 식을 세운다. LCS는 "끝 문자가 같으면 대각선+1, 다르면 위/왼쪽 중 max". Edit Distance는 "같으면 대각선, 다르면 삽입/삭제/교체 중 min+1".
- **LIS**: Longest Increasing Subsequence는 나중에 다른 문제에서 활용된다. O(n²) DP(각 위치에서 "여기로 끝나는 LIS 길이") 또는 O(n log n) 이진탐색(길이별 꼬리 값 유지)으로 풀 수 있다.
