...
- **Stack**: 나중에 넣은 것을 먼저 꺼내는 구조(LIFO). 리스트의 `append`/`pop`으로 구현한다. 괄호 매칭은 여는 괄호를 스택에 넣고, 닫는 괄호가 나오면 top과 짝이 맞는지 보고 pop하는 방식으로 O(n)에 판별할 수 있다.
- **Monotonic Stack**: 스택 안의 원소를 **항상 오름차순(또는 내림차순)**으로 유지하도록 넣을 때 조건을 걸어 빼는 방식이다. "현재 원소보다 다음에 나오는 더 큰(또는 작은) 원소의 위치"를 각 인덱스에 대해 O(n)에 구할 수 있어서, Daily Temperatures 같은 "다음 더 큰 원소까지의 거리" 문제에 쓴다. 스택에는 인덱스를 넣을지 값만 넣을지는 문제에 따라 정한다.
