...
- **BST 불변식**: 모든 노드에서 **왼쪽 서브트리 값 < 루트 < 오른쪽 서브트리 값**이 유지된다. insert/search는 이걸 이용해 크기 비교로 왼쪽/오른쪽을 따라가면 된다.
- **delete 시 대체 노드**: 지울 노드에 **자식이 2개**일 때는, "지울 노드 자리"에 넣어도 불변식이 깨지지 않는 값이 필요하다. **오른쪽 서브트리에서 가장 작은 값**(또는 왼쪽 서브트리에서 가장 큰 값)이 그 역할을 한다. 그 노드를 복사해 두고, 원래 그 노드는 자식이 0개 또는 1개이므로 위의 경우(1)(2)로 제거하면 된다.
